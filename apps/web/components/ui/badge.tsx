...
const badge = cva('inline-flex items-center gap-1 rounded-tile type-label whitespace-nowrap', {
  variants: {
    tone: {
    ...
    size: { sm: 'h-5 px-1.5', md: 'h-[26px] px-2.5' },
    /** Odds pills render in the mono code face (design-system.md §4). */
    mono: { true: 'type-odds text-[11px]', false: '' },
  },
  defaultVariants: { tone: 'neutral', size: 'sm', mono: false },
});

type BadgeProps = HTMLAttributes<HTMLSpanElement> &
  VariantProps<typeof badge> & { icon?: ReactNode };

export function Badge({ className, tone, size, mono, icon, children, ...props }: BadgeProps) {
  return (
